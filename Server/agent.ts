...
import { ToolMessage } from "@langchain/core/messages"         // For tool execution results
...
export async function callAgent(client: MongoClient, query: string, thread_id: string) {
  try {
    ...
    // Create a custom tool execution function
...
Current time: {time}`,
    ...
    const workflow = new StateGraph(GraphState)
      ...
      .addNode("tools", callTools)                    // Add tool execution node
